 ...
 * @return {undefined}
 */
module.exports.changeLayout = ($layout, $init) => {
...
function changeTheme($theme, $style, $themeStylesheet, $init) {
    ...
    if ($theme === 'light') {
        ...
        $themeStylesheet.href = 'build/css/' + $theme + '-' + $style + '.min.css';
    } else if ($theme === 'dark') {
        $logo.attr('src', 'https://via.placeholder.com/334x119');
        $body.removeClass('theme-semidark').addClass('theme-dark');
        $themeStylesheet.href = 'build/css/' + $theme + '-style-1.min.css';
 ...
 * Active fixed style
 */
module.exports.activeFixedStyle = () => {
    var $body = jQuery('body');
...
(function ($) {
    sharedVariables.$themeStylesheet = document.createElement('link');
    sharedVariables.$themeStylesheet.id = 'theme-stylesheet';
    sharedVariables.$themeStylesheet.rel = 'stylesheet';
    sharedVariables.$themeStylesheet.href = '';
    $('head').append(sharedVariables.$themeStylesheet);
